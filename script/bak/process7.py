...
def make_feat(inpath,outpath,isTrain=True):
    ...
    #       "avesize,avesalary,aveworktime,lastposlevel,nextposlevel,descendsize,descendsalary,descendposlevel"
    title="id,major,gender,age,workage,worknum,beforeworktime,posnum,indusnum," \
          "firstsize,firstsalary,lastworktime,lastsize,lastsalary,lastindus,lastposition," \
          "nextworktime,nextsize,nextsalary,nextindus,nextposition,lastposlevel,nextposlevel,descendsize,descendsalary,descendposlevel"
    ...
    for line in fin:
        ...
        lastposlevel,nextposlevel,descendsize,descendsalary,descendposlevel=0,0,0,0,0
        for i in xrange(wn):
        ...
        #           round(float(tsize)/(wn-2)),round(float(tsalary)/(wn-2)),round(float(bworktime)/(wn-2)),
        #           lastposlevel,nextposlevel,descendsize,descendsalary,descendposlevel]
        outlist+=[age-tworktime/12,wn,bworktime,len(posset),len(indusset),
                  firstsize,firstsalary,lastworktime,lastsize,lastsalary,lastindustry,lastposition,
                  nextworktime,nextsize,nextsalary,nextindustry,nextposition,
                  lastposlevel,nextposlevel,descendsize,descendsalary,descendposlevel]
